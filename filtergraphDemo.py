...
parser.add_argument('-s', '--save_config', dest='save_config', default=False, help = 'Save config to config/')
...
cur_graph = None
if args.config == '':
    cur_graph = createFilterGraph(configChoices[config_idx])
else:
    cur_graph = createFilterGraph(args.config)
